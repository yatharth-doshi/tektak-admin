...
const DailyInvestorUser = ({ user, onBack }) => {
    const theme = useTheme();
    const colors = tokens(theme.palette.mode);
    const navigate = useNavigate();

    const handlevideo = () => {
        navigate('/dailyinvestorvideo');
    };
    const handleJobs = () => {
        navigate('/dailyinvestorjobs');
    };
    const handleDailyPodcast = () => {
        navigate('/dailyinvestorpodcast')
    }
    const handleevents = () => {
        navigate('/dailyinvestorevents')
    }
    ...
    return (
        <Box width="90%" mx="auto" my="30px" gap="20px">
            ...
            >
                ...
                <Box backgroundColor={colors.primary[400]} display="flex" alignItems="center" justifyContent="center" boxShadow="0 3px 10px rgba(0, 0, 0, 0.2)" onClick={handleDailyPodcast}>
                    <StatBox
...
export default DailyInvestorUser;
