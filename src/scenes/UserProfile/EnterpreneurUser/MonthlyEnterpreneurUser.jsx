...
const MonthlyEnterpreneurUser = ({ user, onBack }) => {
    const theme = useTheme();
    const colors = tokens(theme.palette.mode);
    const navigate = useNavigate();

    const handledailyEnterpreneur = () => {
        navigate('/monthlyEnterpreneur');
    };
    const handlepodcast = () => {
        navigate('/monthlyenterpreneurpodcast')
    }
    const handlevideo = () => {
        navigate('/monthlyenterpreneurvideo')
    }
    const handlejobs =() => {
        navigate('/monthlyenterpreneurjobs')
    }
    const handleevents =() => {
        navigate('/monthlyenterpreneurevents')
    }
    return (
        <Box width="90%" mx="auto" my="30px" gap="20px">
            ...
            >
                ...
                <Box
                    ...
                    justifyContent="center"
                    boxShadow="0 3px 10px rgba(0, 0, 0, 0.2)"
                    // onClick={handledailyEnterpreneur}
                    onClick={handlevideo}
                >
                    <StatBox
...
export default MonthlyEnterpreneurUser;
